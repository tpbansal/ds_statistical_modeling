...
police_incidents_2 <- police_incidents_1[,c("rms_cdw_id", "summarized_offense_description", "offense_type", "general_offense_number", "offense_code", "offense_code_extension",  "summary_offense_code", "date_reported", "occurred_date_or_date_range_start","month", "year","district_sector", "zone_beat", "hundred_block_location", "census_tract_2000", "location")]
Police_Incident_3 <- na.omit(police_incidents_2)

#Police_Incident_3  # Clean dataset, please check if need be. 

saveRDS(Police_Incident_3, file = "Police_Incident_TapasviB.rds") # Exporting the dataset for future use
...
```{r}
diamonds %>% group_by(cut) %>% summarise( Avg_price = mean(price/carat), Mean_Carat= mean(carat), Mean_Price = mean(price) ) %>% arrange(desc(Avg_price))
...
#ggplot(data=diamonds) + geom_histogram(binwidth = 50, aes(x=price/carat, fill = clarity)) + coord_cartesian(ylim=c(0,600)) + facet_wrap(~cut)

ggplot(data=diamonds) + geom_boxplot(aes (x = clarity, y =price/carat, col = cut)) + coord_cartesian(ylim=c(0,18000))

```
Additional variable of Clarity is used as it's one of the 3 factor variables. It helps to identify how different cuts do with clarity with respect to the price per carat of the diamonds.
The influencing could be the buying pattern of customers who don't understand completely about the nature of diamonds. 
